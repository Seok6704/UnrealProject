...
void UGrabber::OnGrab()
{
	...
	if(HasHit)
	{
		...
		HitComponent->GetOwner()->SetActorRotation(FRotator(0, 0, 0));
		HitComponent->SetCollisionResponseToChannel(ECollisionChannel::ECC_Pawn, ECollisionResponse::ECR_Ignore);
		HitResult.GetActor()->Tags.Add("Grabbed");

		if(CheckTorch(HitResult)) SetTorchTimer(HitResult.GetActor()->GetComponentByClass<UTorchTimer>());
...
void UGrabber::SetTorchTimer(UTorchTimer* NewTorchTimer)
{
	TorchTimer = NewTorchTimer;

	TorchTimer->RunTimer();
}

bool UGrabber::CheckTorch(FHitResult HitResult) const
{
	if(HitResult.GetActor()->ActorHasTag("Torch")) return true;
	else return false;
}
